...
def add_bullet(doc, text, level, continuation_indent=None):
    text = "" if text is None else str(text)
    if not text.strip():
        return None  # evita bullet vuoti e consente al chiamante di controllare
    
    lines = [ln for ln in text.splitlines() if ln is not None]  # manteniamo anche righe vuote intenzionali

    # prima riga: contiene il bullet
    first_line = lines[0] if lines else ""
    para = doc.add_paragraph()
    pf = para.paragraph_format
    pf.left_indent = Pt(12 * level)   # indentazione generale per il livello
    pf.space_after = Pt(2)

    para.add_run("• " + first_line)

    # impostiamo indentazione per righe successive (se non specificata, usiamo la stessa left_indent)
    if continuation_indent is None:
        continuation_indent = Pt(12 * level)

    # righe successive: ciascuna è un paragrafo separato, senza bullet, con indentazione continuation_indent
    for extra in lines[1:]:
        p2 = doc.add_paragraph()
        pf2 = p2.paragraph_format
        pf2.left_indent = continuation_indent
...
def parse_node(node, doc, level=0):
    ...
    match ntype:
        ...
        case "listItem":
            # 1) separa il testo dalle eventuali sottoliste
            text_parts = [
            ...
            # unisci preservando i ritorni a capo originali (non comprimere con spazi)
            raw_text = "\n".join([t for t in text_parts if t is not None and t != ""])

            if raw_text:
                # aggiunge bullet + righe successive rispettando CR/LF
                # puoi modificare continuation_indent se vuoi righe successive più o meno spostate
                add_bullet(doc, raw_text, level, continuation_indent=Pt(12 * level))

            # processa eventuali sottoliste
            for child in node.get("content", []):
