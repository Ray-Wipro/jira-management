...
def add_text(parent, text):
    ...
    match True:
        case _ if isinstance(parent, _Cell):
            p = parent.add_paragraph()
            run = p.add_run(text)
        case _ if isinstance(parent, Paragraph):
            run = parent.add_run(text)
        case _ if isinstance(parent, Document):
            p = parent.add_paragraph()
            run = p.add_run(text)
        case _:
            # fallback generico
        ...
        match node_type:
                    ...
                    for child in node.get("content", []):
                        ...
                        match child["type"]:
                            case "text":
                                ...
                                # Gestione stili (grassetto, corsivo, ecc.)
                ...
                for li in node.get("content", []):
                    for child in li.get("content", []):
                        match child["type"]:
                            ...
                            case "bulletList" | "orderedList":
                                # Ricorsione: gestisci sotto-liste
                                parse_adf_to_docx([child], doc)
                """
                ...
                for li in node.get("content", []):
                    for child in li.get("content", []):
                        match child["type"]:
                            ...
                            case "bulletList" | "orderedList":
                                # Ricorsione anche per elenchi misti/nidificati
                                parse_adf_to_docx([child], doc)
                """
            ...
            case "codeBlock":
                ...
                if code_text.strip():
                    ...
                    run = add_text(parent, code_text.rstrip())
            ...
            case "text":
                ...
                run = add_text(parent, node.get("text", ""))
                if not run:
                    continue
                # gestione stili
